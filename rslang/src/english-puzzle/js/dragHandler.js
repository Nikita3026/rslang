// Drag mouse for puzzle. For game
const drag = {};
document.addEventListener('mousedown', (e) => {
  if (e.target.classList.contains('picked-word-game-puzzle')) {
...
document.addEventListener('mouseup', (e) => {
  if (drag.element) {
    ...
    const isKeyDrag = e.target.getAttribute('puzzle-key-sentence');
    const isKeyDrop = drag.element.getAttribute('puzzle-key-sentencee');
    const roundElements = drag.element.parentNode.classList.contains('game-round-words');

    const isDropPuzzle = e.target.classList.contains('picked-word-game-puzzle');
    if (isDropPuzzle) {
    ...
    if (isDrop &&(isKeyDrag === isKeyDrop) && !e.target.children.length) {
